...
class PortingWorkspaceTests(unittest.TestCase):
    ...
    def test_safe_builtin_tools_execute(self) -> None:
        from src.tools import execute_tool

        read_result = execute_tool('fs.read', 'README.md')
        self.assertTrue(read_result.handled)
        self.assertIn('Baba Code', read_result.message)

        list_result = execute_tool(
            'fs.list',
            json.dumps({'path': '.', 'limit': 10}),
        )
        self.assertTrue(list_result.handled)
        self.assertTrue(list_result.message)

        denied_shell = execute_tool('shell.exec', json.dumps({'command': 'echo unsafe'}))
        self.assertFalse(denied_shell.handled)
        self.assertIn('requires {"confirm": true}', denied_shell.message)

        shell_result = execute_tool(
            'shell.exec',
            json.dumps({'argv': [sys.executable, '-c', 'print("tool-ok")'], 'confirm': True}),
        )
        self.assertTrue(shell_result.handled)
        self.assertIn('tool-ok', shell_result.message)
